...
int main (int argc, char *argv[])
{
  ...
  /*  eoUCPThirdQuadCrossover<IndiS> cross3(tUp, tDown,tColdStart,&data);
   ...
   int nbU = parser.createParam(8, "nbU", "nb unites modifées par mut5", 'u', "Variation Operators").value();
    ...
    moeoAdditiveEpsilonBinaryMetric< moeoUCPObjectiveVector > metric;
    moeoHypervolumeBinaryRelativeRefPointMetric< moeoUCPObjectiveVector > metric1;
  ...
  //  moeoSteadyHyperVolumeContinue<IndiX> continuator( 0, 100, hyperVol);
    int time_max = parser.getORcreateParam(240, "time_max", "time max", 'T', "Algorithm").value();
   ...
   //moeoDecodedNSGAII<IndiS, IndiX> nsgaII(MAX_GEN, decoder, cross,pCross,mut, pMut); 
    ...
    moeoDecodedIBEA <IndiS, IndiX> ibea(MAX_GEN, decoder, cross,pCross,mut, pMut,vectMut,pVectMut, metric1) ;
   ...
   /* if ( algo == 0 ) {
    ...
    {
        for(int j=0;j<pop[i].children().size();j++)
        cout<<pop[i].children()[j]<<endl;
    }*/
