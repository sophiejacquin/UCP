	      ...
	      for(unsigned i=0;i<line.size();i++)
		{
		  ...
		  /* If the character is '@', we have o descrption of a unit features */
		  else if(line[i]=='@')
		    {
		      ...
		      double a0,a1,a2,sHot,sCold, b0, b1, b2;
		      ...
		      b0	   = (double)atof(tVal[cpt++].c_str());
		      b1	   = (double)atof(tVal[cpt++].c_str());
		      b2	   = (double)atof(tVal[cpt++].c_str());
		      ...
		      initialState = atoi(tVal[cpt++].c_str());
                if(initialState>tUp) initialState=tUp;
                tColdStart=tDown+tColdStart+1;
                if(tDown<1) tDown=1;
                
                if(initialState< -tColdStart) initialState=-tColdStart;
                /*Push the unit in  data*/
                initialEtat.push_back(initialState);
		      eoUCPUnit unit(numMachine,Pmin,Pmax,a0,a1,a2,b0,b1,b2,tUp,tDown,sHot,sCold,tColdStart,initialState);
          ...
          _init.set(nb_unites,nb_heures,initialEtat);
      } 
      /*Opening-file fails*/
      else 
	{ 
