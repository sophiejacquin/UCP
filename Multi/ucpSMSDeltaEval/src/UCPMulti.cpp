...
int main (int argc, char *argv[])
{
   ...
   int nbU = parser.createParam(8, "nbU", "nb unites modifées par mut5", 'u', "Variation Operators").value();
    ...
    //moeoHyperVolumeDifferenceMetric< ObjectiveVector, double > metric;
  ...
  //  moeoSteadyHyperVolumeContinue<IndiX> continuator( 0, 100, hyperVol);
    ...
    double nbOffspring=parser.createParam(1.0, "nbOffspring", "Number of Offspring", 'O', "Algorithm" ).value();
    bool isRate=parser.createParam(true, "isRate", "Specify if the unmber of pffspring is a rate", 'R', "Algorithm" ).value();
    eoTimeContinue<IndiX> continuator(time_max);
    moeoDecodedSMS<IndiS, IndiX> sms(MAX_GEN, decoder, cross,pCross,mut, pMut,vectMut,pVectMut, nbOffspring, isRate); 
   ...
   /* if ( algo == 0 ) {
    ...
    cout<<"air de l'hypervolume :"<<-1*valHyperVol<<endl;
        ...
        cout<<"air de l'hypervolume :"<<0<<endl;
   // std::cout << std::endl;
