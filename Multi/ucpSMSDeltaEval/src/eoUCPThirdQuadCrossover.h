...
#ifndef eoUCPThirdQuadCrossover_H
#define eoUCPThirdQuadCrossover_H
...
class eoUCPThirdQuadCrossover: public eoQuadOp<GenotypeT>
{
public:
  /**
   * Ctor - no requirement
...
// START eventually add or modify the anyVariable argument
  eoUCPThirdQuadCrossover(vector<int> & _tUp, vector<int> & _tDown, vector<int> & _tCS,eoUCPData * _data)
	{
		tUp= _tUp;
		tDown= _tDown;
		tColdStart=_tCS;
...
//cout<<"deb cross3"<<endl;
     ...
     //Choix taille fenetre :
     eoUniformGenerator<int> choixTailleF(5,nbHeures-5);
     int tailleF=choixTailleF();
     ...
     {
    	 cheminMin+=_genotype1.getEval(i);
    	 valChemin2+=_genotype2.getEval(i);
     }
     if(valChemin2>cheminMin)mieux=true;
     double valChemin1=cheminMin;
     int TS=0;
     int TE=tailleF;
     // cout<<"av for"<<endl;
     for(i=1;i<nbHeures-tailleF;i++)
     {
    	valChemin1 -=_genotype1.getEval(i);
    	 valChemin2-=_genotype2.getEval(i);
    	 valChemin1+=_genotype1.getEval(i+tailleF);
    	 valChemin2+=_genotype2.getEval(i+tailleF);
    	 if((valChemin1<cheminMin && (valChemin1<valChemin2)==mieux)||(mieux==false && (valChemin1<valChemin2) ))
    	 {
    		 if(valChemin1<valChemin2) mieux=true;
    		 cheminMin=valChemin1;
    		 TS=i;
    		 TE=TS+tailleF;
    	 }
	...
	if(TE+1<nbHeures)
	{
	 ...
	 for(t=TS+1;t<TE+1;t++)
    	 {
		double eval1=_genotype1.getEval(t);
		bool modif1=_genotype1.getModif(t);
    		
		_genotype1.setEval(t,_genotype2.getEval(t));
    		
		_genotype2.setEval(t,eval1);
